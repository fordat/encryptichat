...
initClient :: ClientId -> ClientName -> ClientPass -> Handle -> IO Client
initClient id name pass handle =
    Client <$> return id
           <*> return name
           <*> return pass
           <*> return handle
...
serve server@Server{..} id handle = do
    ...
    hPutStrLn handle "What is your temporary password?"
    pass <- hGetLine handle
    if null name
        then hPutStrLn handle "Bye, anonymous coward"
        else do
            client <- initClient id name pass handle
